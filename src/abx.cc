...
FastDataInput::FastDataInput(std::istream& is) : input_stream(std::ios::binary) {
...
void FastDataOutput::writeInternedUTF(const std::string& str) {
    ...
    if (it != string_pool.end()) {
        ...
        if (interned_strings.size() >= MAX_UNSIGNED_SHORT) {
            throw std::runtime_error("String pool overflow - too many unique strings");
        }
...
const uint8_t BinaryXmlDeserializer::PROTOCOL_MAGIC_VERSION_0[4] = {0x41, 0x42, 0x58, 0x00};

std::string BinaryXmlDeserializer::bytesToHex(const std::vector<uint8_t>& bytes) {
    std::stringstream ss;
...
BinaryXmlDeserializer::BinaryXmlDeserializer(std::istream& is, std::ostream& os) : mOut(os) {
...
BinaryXmlDeserializer::BinaryXmlDeserializer(const std::vector<char>& data, std::ostream& os)
    : mOut(os) {
...
void BinaryXmlSerializer::startDocument() { mOut->writeByte(START_DOCUMENT | TYPE_NULL); }
...
void BinaryXmlSerializer::endTag(const std::string& name) {
    ...
    if (mTagNames[mTagCount - 1] != name) {
        throw std::runtime_error("Mismatched tags: expected '" + mTagNames[mTagCount - 1] +
                                 "', got '" + name + "'");
    }
...
void BinaryXmlSerializer::attributeInterned(const std::string& name, const std::string& value) {
...
void BinaryXmlSerializer::attributeBytesBase64(const std::string& name,
                                               const std::vector<uint8_t>& value) {
    if (value.size() > FastDataOutput::MAX_UNSIGNED_SHORT) {
        throw std::runtime_error("attributeBytesBase64: input size exceeds maximum");
    }
...
void BinaryXmlSerializer::attributeIntHex(const std::string& name, int32_t value) {
    mOut->writeByte(ATTRIBUTE | TYPE_INT_HEX);
    mOut->writeInternedUTF(name);
    mOut->writeInt(value);
}

void BinaryXmlSerializer::attributeLong(const std::string& name, int64_t value) {
    mOut->writeByte(ATTRIBUTE | TYPE_LONG);
    mOut->writeInternedUTF(name);
    mOut->writeLong(value);
}

void BinaryXmlSerializer::attributeLongHex(const std::string& name, int64_t value) {
...
void BinaryXmlSerializer::attributeDouble(const std::string& name, double value) {
    mOut->writeByte(ATTRIBUTE | TYPE_DOUBLE);
    mOut->writeInternedUTF(name);
    mOut->writeDouble(value);
}

void BinaryXmlSerializer::attributeBoolean(const std::string& name, bool value) {
    if (value) {
...
void BinaryXmlSerializer::text(const std::string& text) { writeToken(TEXT, &text); }

void BinaryXmlSerializer::cdsect(const std::string& text) { writeToken(CDSECT, &text); }

void BinaryXmlSerializer::comment(const std::string& text) { writeToken(COMMENT, &text); }
...
void BinaryXmlSerializer::docdecl(const std::string& text) { writeToken(DOCDECL, &text); }

void BinaryXmlSerializer::ignorableWhitespace(const std::string& text) {
    writeToken(IGNORABLE_WHITESPACE, &text);
}

void BinaryXmlSerializer::entityRef(const std::string& text) { writeToken(ENTITY_REF, &text); }
...
bool is_boolean(const std::string& str) { return str == "true" || str == "false"; }
...
void process_xml_node_internal(BinaryXmlSerializer& serializer, const pugi::xml_node& node,
        ...
        case pugi::node_element: {
            ...
            for (const auto& attr : node.attributes()) {
                ...
                if (options.warning_callback &&
                    (attr_name.find("xmlns") == 0 || attr_name.find(':') != std::string::npos)) {
                ...
                if (is_boolean(attr_value)) {
                    ...
                    if (attr_value.length() < 50 &&
                        (attr_value.find(' ') == std::string::npos || attr_value == "true" ||
                         attr_value == "false" || attr_value.find('.') != std::string::npos)) {
