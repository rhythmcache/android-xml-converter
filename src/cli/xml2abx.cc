...
void print_usage() {
  std::cerr << "usage: xml2abx [-i] [--collapse-whitespace] input [output]\n\n"
            ...
            << "  --collapse-whitespace Skip whitespace-only text nodes\n";
}
...
int main(int argc, char *argv[]) {
  ...
  while (arg_idx < argc) {
    if (strcmp(argv[arg_idx], "-i") == 0) {
      in_place = true;
      arg_idx++;
    } else if (strcmp(argv[arg_idx], "--collapse-whitespace") == 0) {
  ...
  try {
    ...
    options.warning_callback = [](const std::string &category,
